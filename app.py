...
class ServerForm(FlaskForm):
    ...
    ssh_port = IntegerField('SSH ПорT', default=22, validators=[DataRequired(), NumberRange(min=1, max=65535)])
    username = StringField('Имя пользователя', validators=[DataRequired()])
    submit = SubmitField('Сохранить')

class GenerateKeyForm(FlaskForm):
    name = StringField('Название ключа', validators=[DataRequired()])
    key_type = SelectField('Тип ключа', choices=[('rsa', 'RSA 4096'), ('ed25519', 'Ed25519')], validators=[DataRequired()])
...
class Server(db.Model):
    ...
    deployments = db.relationship('KeyDeployment', back_populates='server', lazy='dynamic', cascade="all, delete-orphan")
...
class SSHKey(db.Model):
    ...
    deployments = db.relationship('KeyDeployment', back_populates='ssh_key', lazy='dynamic', cascade="all, delete-orphan")
...
class KeyDeployment(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    ssh_key_id = db.Column(db.Integer, db.ForeignKey('ssh_keys.id', ondelete='CASCADE'), nullable=False, index=True)
    ...
    __table_args__ = (
        db.Index('idx_key_server_revoked', 'ssh_key_id', 'server_id', 'revoked_at'),
    )
...
def add_server_with_password():
    ...
    if form.validate_on_submit():
        ...
        try:
            ...
            app.logger.info(f"Сервер инициализирован. OpenSSH версия: {openssh_version}, Legacy: {requires_legacy_ssh}")
        ...
        deploy_result = ssh_manager.deploy_key_with_password(ip_address, port, username, password, public_key_ssh)
        ...
        try:
            ...
            db.session.commit()
            app.logger.info(f"Создан KeyDeployment: ключ {new_root_key.id} -> сервер {new_server.id}")
        except Exception as e:
            flash(f'Ошибка при создании записи развертывания: {e}', 'danger')
            db.session.rollback()
            return redirect(url_for('servers'))
        
        add_log('add_server', target=new_server.name, details={'ip': new_server.ip_address, 'key_id': new_root_key.id, 'openssh_version': openssh_version})
    ...
    try:
        ...
        for line in lines:
            ...
            try:
                ...
                init_result = ssh_manager.initialize_server(ip_address, ssh_port, username, password)
                
                if not init_result['success']:
                    app.logger.warning(f'⚠️ Ошибка инициализации {domain}: {init_result.get("message")}')
                    failed += 1
                    continue
                
                openssh_version = init_result.get('openssh_version', 'unknown')
                requires_legacy_ssh = init_result.get('requires_legacy_ssh', False)
                app.logger.info(f'✅ Сервер {domain}: OpenSSH {openssh_version}, legacy={requires_legacy_ssh}')
            ...
            try:
                ...
                app.logger.info(f'Создан сервер {domain} (ID: {new_server.id}), OpenSSH: {openssh_version}, Legacy: {requires_legacy_ssh}')
            ...
            add_log('add_server', target=domain, details={'ip': ip_address, 'port': ssh_port, 'key_id': new_root_key.id})
            added += 1
...
def keys():
    ...
    return render_template('keys.html', 
                           ...
                           keys=user_keys, servers=user_servers)
...
def deploy_key_route():
    ...
    try:
        ...
        if key_to_deploy.user_id != current_user.id or server.user_id != current_user.id:
            app.logger.warning(f'Попытка несанкционированного доступа к ключу {key_id} или серверу {server_id}')
            return jsonify({'success': False, 'message': 'Доступ запрещен'}), 403

        # НАЙТИ УНИКАЛЬНЫЙ ключ для подключения: root_{server.name}
        root_key_name = f"root_{server.name}"
        access_key = SSHKey.query.filter_by(
            user_id=current_user.id,
            name=root_key_name
        ).first()
        ...
        try:
            ...
            return jsonify({'success': False, 'message': f'Ошибка расшифровки ключа: {str(e)}'}), 500

        # Развернуть ключ на сервер с адаптивными алгоритмами
        try:
            app.logger.info(f'Развертывание {key_to_deploy.name} на {server.ip_address}:{server.ssh_port}')
            ...
            if success:
                ...
                if not existing:
                    ...
                    db.session.add(deployment)
                    db.session.commit()
                    app.logger.info(f'Запись KeyDeployment создана для ключа {key_id} на сервере {server_id}')
                else:
                    app.logger.info(f'Запись KeyDeployment уже существует для ключа {key_id} на сервере {server_id}')
...
def test_server_connection(server_id):
    ...
    if not server.access_key:
        return jsonify({'success': False, 'message': 'Для этого сервера не настроен ключ доступа. Пожалуйста, пересоздайте сервер.'})
    
    access_key = server.access_key

    encryption_key = os.environ.get('ENCRYPTION_KEY')
    try:
        private_key = ssh_manager.decrypt_private_key(access_key.private_key_encrypted, encryption_key)
    except Exception:
        return jsonify({'success': False, 'message': 'Не удалось дешифровать ключ доступа \'access_key\'.'})
    ...
    add_log('test_connection', target=server.name, details={'result': 'success' if success else 'failed'})
...
def get_key_servers(key_id):
    ...
    try:
        ...
        deployments = KeyDeployment.query.filter_by(
            ssh_key_id=key_id,
            revoked_at=None
        ).all()
    ...
    try:
        ...
        if not access_key:
            ...
            try:
                ...
                private_key_decrypted = ssh_manager.decrypt_private_key(root_key.private_key_encrypted, encryption_key)
                ...
                if success:
                    ...
                    add_log('revoke_key', target=ssh_key.name, details={'server': server.name, 'status': 'revoked'})
                ...
                return jsonify({'success': False, 'error': f'Ошибка при отзыве ключа: {str(e)}'}), 500
                ...
                try:
                    ...
                    try:
                        private_key_decrypted = ssh_manager.decrypt_private_key(
                            root_key.private_key_encrypted,
                            encryption_key
                        )
                    except Exception as e:
                        app.logger.error(f'Ошибка при расшифровке root ключа для {server.name}: {str(e)}')
                        return {
                            'server_id': server.id,
                            'server_name': server.name,
                            'success': False,
                    ...
                    try:
                        ...
                        if success:
                            ...
                            add_log('revoke_key_all', target=ssh_key_name, 
                            ...
                            app.logger.warning(f'Ошибка при отзыве ключа с {server.name}: {message}')
                            add_log('revoke_key_all_failed', target=ssh_key_name,
...
def revoke_key_deployment():
    ...
    if server_id is None:
        servers_with_key = Server.query.join(KeyDeployment).filter(
        ...
        if not servers_with_key:
            return jsonify({'success': True, 'message': 'Ключ не был развернут ни на одном сервере.'})
        ...
        for sid in report['success']:
            deployment = KeyDeployment.query.filter_by(ssh_key_id=key_id, server_id=sid, revoked_at=None).first()
            if deployment:
                deployment.revoked_at = db.func.now()
                deployment.revoked_by = current_user.id
                add_log('revoke_key', target=key_to_revoke.name, details={'server_id': sid, 'result': 'success'})
        db.session.commit()
    ...
    # Сценарий 2: Отозвать с одного сервера
        ...
        access_key = server.access_key
        if not access_key:
            return jsonify({'success': False, 'message': 'Ключ доступа для сервера не найден.'}), 500
        ...
        try:
            ...
            if result['success']:
                ...
                deployment.revoked_by = current_user.id
                db.session.commit()
                add_log('revoke_key', target=key_to_revoke.name, details={'server': server.name, 'result': 'success', 'openssh_version': server.openssh_version})
                return jsonify({'success': True, 'message': 'Ключ успешно отозван.'})
            else:
                add_log('revoke_key_failed', target=key_to_revoke.name, details={'server': server.name, 'error': result['message']})
...
def logs():
    ...
    logs_pagination = Log.query.filter_by(user_id=current_user.id)\
                             .order_by(Log.timestamp.desc())\
