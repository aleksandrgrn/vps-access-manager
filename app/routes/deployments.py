    ...
    try:
        ...
        for deployment in active_deployments:
            ...
            if server and server.access_key:
                ...
                logger.warning(
                    f"[REVOKE_ALL] Skipping server {server.name if server else 'Unknown'} "
                    "- no access_key"
                )
        ...
        logger.info(
            f"[REVOKE_ALL_SUCCESS] Revoked from {bulk_result['success_count']}/"
        ...
        return (
            jsonify(
                {
                    "success": True,
                    "message": f'✅ Отозвано с {bulk_result["success_count"]} '
                    f"из {len(servers)} серверов",
                    "total": len(servers),
    ...
    try:
        ...
        return (
            jsonify(
                {
                    "success": True,
                    "message": f"Развёртывание завершено: {success_count} успешно, "
