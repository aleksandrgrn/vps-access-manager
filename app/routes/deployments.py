...
from app import db
from app.models import Server, SSHKey, KeyDeployment
from app.utils import add_log
from app.services.key_service import decrypt_access_key, revoke_key_from_single_server, revoke_key_from_all_servers, deploy_key_to_server
    ...
    try:
        ...
        if deployment_id:
            ...
            if not decrypt_result['success']:
                ...
                add_log('revoke_key_decrypt_failed', target=key_to_revoke.name, details={
                    'server': server.name,
                    'error': decrypt_result['message']
                })
                return jsonify({
                    'success': False,
            ...
            try:
                ...
                add_log('revoke_key_exception', target=key_to_revoke.name, details={
                    'server': server.name,
                    'error': str(ssh_error)
                })
                return jsonify({
            ...
            if not revoke_result['success']:
                ...
                add_log('revoke_key_failed', target=key_to_revoke.name, details={
                    'server': server.name,
            ...
            try:
                ...
                add_log('revoke_key', target=key_to_revoke.name, details={
                    'server': server.name,
                    'result': 'success'
                })
        ...
        # ========== SCENARIO 1: Отзыв со всех серверов ==========
            ...
            if not active_deployments:
                return jsonify({
                    'success': False,
                    'message': 'Нет активных развёртываний для этого ключа'
                }), 400
            ...
            for result in bulk_result['results']:
                if result['success']:
                    deployment = KeyDeployment.query.filter_by(
                        ssh_key_id=key_id,
                        server_id=next((s.id for s in servers if s.name == result['server_name']), None),
                        revoked_at=None
                    ).first()
            ...
            add_log('revoke_key_bulk', target=key_to_revoke.name, details={
        ...
        # ========== SCENARIO 2: Отзыв с одного сервера ==========
            ...
            if not deployment:
                return jsonify({
                    'success': False,
                    'message': 'Активное развёртывание не найдено'
                }), 404
            ...
            return jsonify({
                'success': True,
                'message': f'✅ Ключ отозван с {server.name}'
            }), 200
...
def revoke_key_all() -> Tuple[Dict[str, Any], int]:
    ...
    try:
        ...
        # Проверка доступа
        if key_to_revoke.user_id != current_user.id:
            logger.error(f"[REVOKE_ALL] Access denied: user_id={current_user.id}, key_user_id={key_to_revoke.user_id}")
        ...
        for deployment in active_deployments:
            ...
            if server and server.access_key:
                servers.append(server)
                access_keys[server.id] = server.access_key
                logger.debug(f"[REVOKE_ALL] Added server {server.name} (access_key_id={server.access_key.id})")
            else:
                logger.warning(f"[REVOKE_ALL] Skipping server {server.name if server else 'Unknown'} - no access_key")
        ...
        logger.info(f"[REVOKE_ALL_SUCCESS] Revoked from {bulk_result['success_count']}/{len(servers)} servers")
    ...
    try:
        ...
        for deployment in deployments:
            results.append({
                ...
                'revoked_at': deployment.revoked_at.isoformat() if deployment.revoked_at else None,
...
def deploy_key() -> Tuple[Dict[str, Any], int]:
    ...
    try:
        ...
        logger.info(f"[DEPLOY_KEY_START] Развёртывание ключа {key.name} на {len(server_ids)} серверов")
        ...
        for server_id in server_ids:
            try:
                ...
                decrypt_result = decrypt_access_key(server.access_key)
                if not decrypt_result['success']:
                    logger.error(f"[DEPLOY_KEY_DECRYPT_FAILED] {server.name}: {decrypt_result['message']}")
                    results.append({
                        'server_id': server_id,
                        'server_name': server.name,
                        'success': False,
                ...
                deploy_result = deploy_key_to_server(
                    server,
                    decrypt_result['private_key'],
                    key
                )
        ...
        add_log('deploy_key', target=key.name, details={
