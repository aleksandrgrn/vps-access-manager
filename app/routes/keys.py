...
def generate_key() -> Any:
    ...
    try:
        # Генерация ключа
        private_key_pem, public_key_ssh = ssh.generate_ssh_key(form.key_type.data)
        fingerprint = ssh.get_fingerprint(public_key_ssh)
        ...
        if existing_key:
            ...
            flash(
                f'ℹ️ Ключ с таким fingerprint уже существует: "{existing_key.name}". '
            ...
            encrypted_private_key = ssh.encrypt_private_key(private_key_pem, encryption_key)
...
def upload_key() -> Any:
    ...
    try:
        ...
        if not ssh.validate_ssh_public_key(public_key):
            flash("Неверный формат публичного ключа.", "danger")
            return redirect(url_for("keys.keys"))

        # Вычисление fingerprint
        fingerprint = ssh.get_fingerprint(public_key)
        if not fingerprint:
            flash("Не удалось вычислить fingerprint ключа.", "danger")
            return redirect(url_for("keys.keys"))
        ...
        if existing_key:
            ...
            encryption_key = os.environ.get("ENCRYPTION_KEY")
            empty_encrypted = ssh.encrypt_private_key("", encryption_key)
...
def deploy_key() -> Tuple[Dict[str, Any], int]:
    ...
    try:
        ...
        try:
            ...
            flash(f"✅ Ключ успешно развёрнут на {server.name}", "success")
...
def bulk_deploy_key() -> Tuple[Dict[str, Any], int]:
    ...
    try:
        ...
        if key.user_id != current_user.id:
            logger.warning(
                f"[BULK_DEPLOY] Попытка доступа к чужому ключу: "
                f"user={current_user.id}, key_owner={key.user_id}"
            )
            return jsonify({"success": False, "message": "Доступ запрещён"}), 403
        ...
        if servers_without_access_key:
            return (
                jsonify(
                    {
                        "success": False,
                        "message": f"Серверы без ключа доступа: "
        ...
        results = ssh.deploy_key_to_multiple_servers(key, servers, encryption_key)
