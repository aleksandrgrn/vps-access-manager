...
def keys() -> str:
    ...
    try:
        ...
        return render_template('keys.html', generate_form=GenerateKeyForm(), upload_form=UploadKeyForm(), keys=[], servers=[])
...
def generate_key() -> Any:
    ...
    try:
        ...
        if existing_key:
            ...
            logger.info(f"[KEY_EXISTS] Ключ с fingerprint {fingerprint} уже существует (ID: {existing_key.id}, имя: '{existing_key.name}')")
            flash(f'ℹ️ Ключ с таким fingerprint уже существует: "{existing_key.name}". Будет использован существующий ключ.', 'info')
            key_to_use = existing_key
        else:
            # Проверка имени (только если создаём новый ключ)
            existing_name = SSHKey.query.filter_by(user_id=current_user.id, name=form.name.data).first()
            if existing_name:
                flash(f'❌ Ключ с именем "{form.name.data}" уже существует. Выберите другое имя.', 'danger')
                return redirect(url_for('keys.keys'))
...
def deploy_key() -> Tuple[Dict[str, Any], int]:
    ...
    try:
        ...
        if existing_deployment:
            return jsonify({
                'success': False,
                'message': f'Ключ уже развёрнут на сервере {server.name}'
            }), 400
        ...
        try:
            ...
            return jsonify({
                'success': False,
                'message': f'SSH успешно, но ошибка БД: {str(db_error)}'
            }), 500
...
def view_key(key_id: int) -> Tuple[Dict[str, Any], int]:
    ...
    try:
        ...
        # Проверка доступа
        if ssh_key.user_id != current_user.id:
            logger.error(f"[VIEW_KEY] Access denied: user_id={current_user.id}, key_user_id={ssh_key.user_id}")
...
def bulk_deploy_key() -> Tuple[Dict[str, Any], int]:
    ...
    try:
        ...
        if not server_ids or not isinstance(server_ids, list):
            return jsonify({'success': False, 'message': 'server_ids должен быть непустым списком'}), 400
        ...
        if key.user_id != current_user.id:
            logger.warning(f"[BULK_DEPLOY] Попытка доступа к чужому ключу: user={current_user.id}, key_owner={key.user_id}")
            return jsonify({'success': False, 'message': 'Доступ запрещён'}), 403
        ...
        if not servers:
            return jsonify({'success': False, 'message': 'Не найдено ни одного доступного сервера'}), 404
        ...
        logger.info(f"[BULK_DEPLOY] Начало массового развертывания ключа {key.name} на {len(servers)} серверов")
        ...
        for deployed in results['deployed']:
            try:
                ...
                logger.error(f"[BULK_DEPLOY] Ошибка БД для сервера {deployed['server_name']}: {str(db_error)}")
