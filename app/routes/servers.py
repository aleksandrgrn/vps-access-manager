...
def add_server() -> Any:
    ...
    try:
        ...
        init_result = ssh.initialize_server(ip_address, port, username, password)
    ...
    try:
        ...
        # Выбор типа ключа в зависимости от версии сервера
        key_type = "rsa" if requires_legacy_ssh else "ed25519"
        logger.info(f"[ADD_SERVER] Тип ключа: {key_type} (Legacy: {requires_legacy_ssh})")

        private_key_pem, public_key_ssh = ssh.generate_ssh_key(key_type)
    ...
    try:
        ...
        deploy_result = ssh.deploy_key_with_password(
    ...
    try:
        ...
        for line in lines:
            ...
            try:
                ...
                init_result = ssh.initialize_server(ip_address, ssh_port, username, password)
            ...
            try:
                ...
                if existing_key:
                    ...
                    private_key_pem, public_key_ssh = ssh.generate_ssh_key("rsa")
                    fingerprint = ssh.get_fingerprint(public_key_ssh)
