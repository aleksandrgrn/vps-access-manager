...
def add_server() -> Any:
    ...
    try:
        ...
        requires_legacy_ssh = form.requires_legacy_ssh.data if form.requires_legacy_ssh.data else init_result['requires_legacy_ssh']
        
        logger.info(f"[ADD_SERVER] Сервер инициализирован. OpenSSH: {openssh_version}, Legacy: {requires_legacy_ssh}")
    ...
    try:
        ...
        db.session.commit()
        logger.info(f"[ADD_SERVER] KeyDeployment создан: ключ {new_root_key.id} -> сервер {new_server.id}")
        
        add_log('add_server', target=new_server.name, details={
...
def edit_server(server_id: int) -> Tuple[Dict[str, Any], int]:
    ...
    try:
        ...
        if form.validate_on_submit():
            ...
            logger.info(f"[EDIT_SERVER] Сервер {server.name} обновлён. Legacy SSH: {server.requires_legacy_ssh}")
            add_log('edit_server', target=server.name, details={'requires_legacy_ssh': server.requires_legacy_ssh})
...
def test_server(server_id: int) -> Tuple[Dict[str, Any], int]:
    ...
    try:
        ...
        add_log('test_connection', target=server.name, details={
    ...
    try:
        ...
        for line in lines:
            ...
            try:
                ...
                if not init_result['success']:
                    logger.warning(f'[BULK_IMPORT] Ошибка инициализации {domain}: {init_result.get("message")}')
                    failed += 1
                    continue
                
                openssh_version = init_result.get('openssh_version', 'unknown')
                requires_legacy_ssh = init_result.get('requires_legacy_ssh', False)
                logger.info(f'[BULK_IMPORT] {domain}: OpenSSH {openssh_version}, legacy={requires_legacy_ssh}')
            ...
            try:
                ...
                if existing_key:
                    logger.info(f"[BULK_IMPORT] {domain}: используется существующий ключ '{existing_key.name}' (ID: {existing_key.id})")
                    ...
                    db.session.add(new_root_key)
                    db.session.flush()
                    logger.info(f'[BULK_IMPORT] Создан ключ {root_key_name} (ID: {new_root_key.id})')
            ...
            try:
                ...
                db.session.add(deployment)
                db.session.commit()
                logger.info(f'[BULK_IMPORT] KeyDeployment создан: ключ {new_root_key.id} -> сервер {new_server.id}')
            ...
            add_log('add_server', target=domain, details={
                'ip': ip_address,
                'port': ssh_port,
                'key_id': new_root_key.id
            })
...
def logs() -> str:
    ...
    return render_template('logs.html', logs_pagination=logs_pagination, action_colors=action_colors)
