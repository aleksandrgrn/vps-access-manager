...
def decrypt_access_key(access_key: "SSHKey") -> Dict[str, Any]:
    ...
    try:
        ...
        private_key = ssh.decrypt_private_key(access_key.private_key_encrypted, encryption_key)
...
def revoke_key_from_single_server(
    ...
    try:
        ...
        # Вызов SSH операции с передачей server объекта
        result = ssh.revoke_key(
            ip=server.ip_address,
            port=server.ssh_port,
            username=server.username,
            private_key_str=private_key,
            public_key_to_revoke=key_to_revoke.public_key,
            server=server,
        )
...
def deploy_key_to_server(server: Server, private_key: str, key_to_deploy: SSHKey) -> Dict[str, Any]:
    ...
    try:
        success, message = ssh.deploy_key(
            ip=server.ip_address,
            port=server.ssh_port,
            username=server.username,
            private_key_str=private_key,
