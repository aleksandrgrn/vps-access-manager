...
def test_connection(
    ip: str, port: int, username: str, private_key_str: str, server=None
) -> Tuple[bool, str]:
    """
    ...
    try:
        ...
        if server:
            logger.info(
                f"[TEST_CONNECTION] Тестирование {ip}:{port} с адаптивными алгоритмами (OpenSSH: {getattr(server, 'openssh_version', 'unknown')}, Legacy: {getattr(server, 'requires_legacy_ssh', False)})"
            )
            key_file = None
            try:
                key_file = io.StringIO(private_key_str)
...
def add_key_to_authorized_keys(
    host: str, port: int, username: str, password: str, public_key: str
) -> Tuple[bool, str]:
    """
...
def revoke_key_from_server(
    server_ip: str, port: int, username: str, private_key_str: str, public_key: str
) -> Tuple[bool, str]:
    """
        ...
        try:
            ...
            try:
                stdin, stdout, stderr = client.exec_command(
                    f"mv {backup_path} {authorized_keys_path}"
                )
...
def revoke_key_detailed(
    server_ip: str, ssh_port: int, username: str, private_key_str: str, public_key: str, server=None
) -> Tuple[bool, str, str]:
    """
    ...
    if use_legacy_method:
        logger.info(
            f"[REVOKE_LEGACY_MODE] {server_ip}:{ssh_port} OpenSSH={openssh_version} → using legacy compatible method"
        )
    else:
        logger.info(
            f"[REVOKE_MODERN_MODE] {server_ip}:{ssh_port} OpenSSH={openssh_version} → using standard method"
        )
    try:
        client = None
        try:
            ...
            if server:
                ...
                try:
                    ...
                    client = connect_with_adaptive_algorithms(
                        server_ip, ssh_port, username, pkey, server
                    )
                finally:
                    key_file.close()
            else:
        ...
        try:
            ...
            try:
                logger.info(f"Создание backup файла на {server_ip}")
                stdin, stdout, stderr = client.exec_command(
                    f"cp {authorized_keys_path} {backup_path}"
                )
                if stdout.channel.recv_exit_status() != 0:
                    logger.warning(f"Не удалось создать backup на {server_ip}")
            except Exception as e:
            ...
            try:
                ...
                return (
                    True,
                    "Файл authorized_keys не найден, ключ уже считается отозванным.",
                    "SUCCESS",
                )
...
def deploy_key(
    ip: str, port: int, username: str, private_key_str: str, public_key_to_deploy: str, server=None
) -> Tuple[bool, str]:
    """
    ...
    try:
        ...
        if server:
            logger.info(
                f"Развертывание ключа на {ip}:{port} с адаптивными алгоритмами (OpenSSH: {getattr(server, 'openssh_version', 'unknown')}, Legacy: {getattr(server, 'requires_legacy_ssh', False)})"
            )
            key_file = None
            try:
                key_file = io.StringIO(private_key_str)
            ...
            # Обычное подключение без адаптивных алгоритмов
            logger.info(
                f"Развертывание ключа на {ip}:{port} без адаптивных алгоритмов (информация о сервере недоступна)"
            )
            client = _get_ssh_client(ip, port, username, private_key_str)
...
def revoke_key_from_all_servers(public_key: str, all_servers: List, user_credentials: Dict) -> Dict:
    ...
    results = {"success": [], "failed": [], "total": len(all_servers), "revoked": 0}
        ...
        try:
            ...
            try:
                private_key = decrypt_private_key(
                    access_key.private_key_encrypted, user_credentials["encryption_key"]
                )
            except Exception as e:
    ...
    try:
        with ThreadPoolExecutor(max_workers=10) as executor:
            future_to_server = {
                executor.submit(revoke_task, server): server for server in all_servers
            }
    ...
    logger.info(
        f"Отзыв ключа завершен. Успешно: {results['revoked']}, Ошибок: {len(results['failed'])}"
    )
    return results
...
def connect_with_adaptive_algorithms(
    ...
    try:
        ...
        if (
            ...
            logger.info(
                f"🔧 Использование legacy SSH алгоритмов для {ip}:{port} (OpenSSH версия: {getattr(server_obj, 'openssh_version', 'unknown')})"
            )
...
def deploy_key_to_multiple_servers(key_to_deploy, servers: List, encryption_key: str) -> Dict:
    ...
    def deploy_task(server):
        ...
        try:
            ...
            if not access_key:
                logger.warning(
                    f"[BULK_DEPLOY] Ключ доступа для сервера {server.id} ({server.name}) не найден"
                )
            ...
            try:
                ...
                logger.error(
                    f"[BULK_DEPLOY] Ошибка при дешифровке ключа для сервера {server.id}: {e}"
                )
                return server.id, server.name, False, f"Ошибка при дешифровке ключа: {str(e)}"

            logger.info(
                f"[BULK_DEPLOY] Развертывание ключа на сервер {server.id} ({server.name}, {server.ip_address})"
            )
            ...
            logger.error(
                f"[BULK_DEPLOY] Критическая ошибка при развертывании на сервер {server.id}: {str(e)}"
            )
        ...
        with ThreadPoolExecutor(max_workers=10) as executor:
            ...
            for future in as_completed(future_to_server, timeout=300):
                try:
                    ...
                    server = future_to_server[future]
                    logger.error(
                        f"[BULK_DEPLOY] ⏱️ Таймаут при развертывании на сервер {server.name}"
                    )
                    results["failed"].append(
                        {
                            "server_id": server.id,
                    ...
                    server = future_to_server[future]
                    logger.error(
                        f"[BULK_DEPLOY] Ошибка при обработке результата для сервера {server.name}: {e}"
                    )
                    results["failed"].append(
    ...
    logger.info(
        f"[BULK_DEPLOY] Завершено. Успешно: {len(results['deployed'])}, Ошибок: {len(results['failed'])}"
    )
    return results
...
def revoke_key(ip, port, username, private_key_str, public_key_to_revoke, server):
    ...
    try:
        logger.info(
            f"Начинаем удаление ключа с сервера {ip}:{port} (OpenSSH: {getattr(server, 'openssh_version', 'unknown')}, Legacy: {getattr(server, 'requires_legacy_ssh', False)})"
        )
